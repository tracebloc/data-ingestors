    ...
    try:
        ...
        for i, col in enumerate(df.columns):
            ...
            if col != stripped_col:
                ...
                special_chars = [
                    c for c in col if not c.isalnum() and c not in ["_", "-"]
                ]
                if special_chars:
                    print(
                        f"   ⚠️  Column {i}: Special characters found: {original_col} -> {special_chars}"
                    )
                    issues_found = True
        ...
        for col in error_columns:
            if col in df.columns:
                ...
                print(
                    f"      Sample value: {df[col].iloc[0] if len(df) > 0 else 'N/A'}"
                )
                ...
                similar = [
                    c for c in df.columns if col.replace("_", "") in c.replace("_", "")
                ]
                if similar:
