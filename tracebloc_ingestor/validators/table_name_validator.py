...
class TableNameValidator(BaseValidator):
    ...
    def _validate_table_names(self, table_names: List[str]) -> ValidationResult:
        ...
        # Create progress bar
        progress_bar = self._create_progress_bar(len(table_names), "Validating table names")
        ...
        try:
            for table_name in table_names:
                ...
                if not self.pattern.match(table_name):
                    invalid_names.append(f"'{table_name}' (contains invalid characters)")
                    continue
    ...
    def _is_reserved_keyword(self, table_name: str) -> bool:
        ...
        reserved_keywords = {
            ...
            'as', 'order', 'group', 'by', 'having', 'union', 'join', 'inner', 'left',
            'right', 'outer', 'full', 'cross', 'natural', 'on', 'using', 'case',
            'when', 'then', 'else', 'end', 'if', 'exists', 'all', 'any', 'some',
            'distinct', 'top', 'limit', 'offset', 'fetch', 'with', 'recursive',
            'window', 'over', 'partition', 'rows', 'range', 'preceding', 'following',
            'current', 'row', 'unbounded', 'first', 'last', 'value', 'values',
            'set', 'into', 'values', 'returning', 'begin', 'commit', 'rollback',
            'transaction', 'savepoint', 'release', 'lock', 'unlock', 'grant',
            'revoke', 'deny', 'exec', 'execute', 'sp_', 'xp_', 'fn_', 'dt_',
            'user', 'role', 'schema', 'catalog', 'session', 'system', 'public',
            'private', 'protected', 'internal', 'external', 'temporary', 'temp'
        }
