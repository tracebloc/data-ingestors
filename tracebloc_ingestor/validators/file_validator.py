...
class FileTypeValidator(BaseValidator):
    ...
    def validate(self, path: Any, **kwargs) -> ValidationResult:
        ...
        try:
            ...
            files_to_validate = self._get_files_to_validate(data, recursive, ignore_hidden)
    ...
    def _get_files_to_validate(self, data: Any, recursive: bool, ignore_hidden: bool) -> List[Path]:
    ...
    def _validate_file_extensions(self, files: List[Path]) -> ValidationResult:
        ...
        if len(extensions) > 1:
            return self._create_result(
                is_valid=False,
                errors=[f"Multiple file extensions found: {sorted(extensions)}. All files must have the same extension.",
