...
class ImageResolutionValidator(BaseValidator):
    ...
    def set_expected_resolution(self, resolution: Tuple[int, int]):
        """Update the expected resolution for validation.
        
        Args:
            resolution: Expected image resolution as (width, height)
        """
        self.expected_resolution = resolution
        logger.info(f"Updated expected resolution: {resolution}")
    
    def set_tolerance(self, tolerance: int):
        """Update the tolerance for resolution differences.
        
        Args:
            tolerance: Tolerance for resolution differences
        """
        self.tolerance = tolerance
        logger.info(f"Updated resolution tolerance: {tolerance}")
    
    def add_supported_format(self, format_ext: str):
        """Add a new supported image format.
        
        Args:
            format_ext: Image format extension to add (with or without leading dot)
        """
        normalized_format = format_ext.lower() if format_ext.startswith('.') else f'.{format_ext.lower()}'
        self.supported_formats.add(normalized_format)
        logger.info(f"Added supported image format: {normalized_format}")
    
    def remove_supported_format(self, format_ext: str):
        """Remove a supported image format.
        
        Args:
            format_ext: Image format extension to remove
        """
        normalized_format = format_ext.lower() if format_ext.startswith('.') else f'.{format_ext.lower()}'
        self.supported_formats.discard(normalized_format)
        logger.info(f"Removed supported image format: {normalized_format}")
