...
class PascalVOCXMLValidator(BaseValidator):
    ...
    def __init__(self, 
                 name: str = "Pascal VOC XML Validator",
        ...
        self.required_source_elements = {
            'database', 'annotation'
        }
        
        # Required size sub-elements
        self.required_size_elements = {
            'width', 'height', 'depth'
        }
    ...
    def _get_xml_files(self, data: Any, recursive: bool, ignore_hidden: bool) -> List[Path]:
    ...
    def _validate_xml_files(self, files: List[Path]) -> ValidationResult:
        ...
        # Create progress bar
        progress_bar = self._create_progress_bar(len(files), "Validating Pascal VOC XML files")
        ...
        try:
            for file_path in files:
                ...
                all_errors.extend([f"{file_path.name}: {error}" for error in file_result.errors])
                all_warnings.extend([f"{file_path.name}: {warning}" for warning in file_result.warnings])
    ...
    def _validate_source_element(self, root: ET.Element) -> Dict[str, Any]:
        ...
        missing_source_elements = self.required_source_elements - present_source_elements
        
        if missing_source_elements:
            errors.append(f"Missing required source elements: {sorted(missing_source_elements)}")
    ...
    def _validate_size_element(self, root: ET.Element) -> Dict[str, Any]:
        ...
        if missing_size_elements:
            errors.append(f"Missing required size elements: {sorted(missing_size_elements)}")
        ...
        for elem_name in ['width', 'height', 'depth']:
            ...
            if elem is not None:
                try:
                    ...
                    if value <= 0:
                        errors.append(f"{elem_name} must be a positive integer, found: {value}")
                    else:
                        metadata[elem_name] = value
                except (ValueError, TypeError):
                    errors.append(f"{elem_name} must be a valid integer, found: {elem.text}")
    ...
    def _validate_single_object(self, obj: ET.Element, index: int) -> Dict[str, Any]:
        ...
        if missing_obj_elements:
            errors.append(f"Object {index}: Missing required elements: {sorted(missing_obj_elements)}")
        
        # Validate name element
        name_elem = obj.find('name')
        if name_elem is not None:
            if name_elem.text is None or name_elem.text.strip() == '':
                errors.append(f"Object {index}: Name element must have non-empty text content")
            else:
        ...
        if pose_elem is not None:
            if pose_elem.text is None or pose_elem.text.strip() == '':
                errors.append(f"Object {index}: Pose element must have non-empty text content")
            else:
        ...
        if truncated_elem is not None:
            if truncated_elem.text not in ['0', '1']:
                errors.append(f"Object {index}: Truncated element must be '0' or '1', found: {truncated_elem.text}")
            else:
        ...
        if difficult_elem is not None:
            if difficult_elem.text not in ['0', '1']:
                errors.append(f"Object {index}: Difficult element must be '0' or '1', found: {difficult_elem.text}")
            else:
    ...
    def _validate_bndbox_element(self, obj: ET.Element, index: int) -> Dict[str, Any]:
        ...
        missing_bndbox_elements = self.required_bndbox_elements - present_bndbox_elements
        
        if missing_bndbox_elements:
            errors.append(f"Object {index}: Missing required bndbox elements: {sorted(missing_bndbox_elements)}")
        ...
        for coord_name in ['xmin', 'ymin', 'xmax', 'ymax']:
            ...
            if coord_elem is not None:
                try:
                    ...
                    if value < 0:
                        errors.append(f"Object {index}: {coord_name} must be non-negative, found: {value}")
                    else:
                        coords[coord_name] = value
                except (ValueError, TypeError):
                    errors.append(f"Object {index}: {coord_name} must be a valid integer, found: {coord_elem.text}")
            elif coord_name in self.required_bndbox_elements:
                errors.append(f"Object {index}: Missing required '{coord_name}' element")
        
        # Validate coordinate relationships
        if len(coords) == 4:
            if coords['xmin'] >= coords['xmax']:
                errors.append(f"Object {index}: xmin ({coords['xmin']}) must be less than xmax ({coords['xmax']})")
            if coords['ymin'] >= coords['ymax']:
                errors.append(f"Object {index}: ymin ({coords['ymin']}) must be less than ymax ({coords['ymax']})")
            ...
            if area == 0:
                ...
                warnings.append(f"Object {index}: Very small bounding box (area: {area})")
