...
def test_data_validator_date_exception(monkeypatch):
    ...
    with patch(
        "tracebloc_ingestor.validators.data_validator.pd.to_datetime",
...
def test_duplicate_check_directory_exists_exception():
    v = DuplicateValidator(dest_path="/x")
    with patch(
        "tracebloc_ingestor.validators.duplicate_validator.Path",
...
# ===========================================================================
# table_name / keypoint / numeric: outer except
# ===========================================================================
...
def _client(**ov):
    d = dict(
        BACKEND_TOKEN="tok", CLIENT_USERNAME=None, CLIENT_PASSWORD=None, EDGE_ENV="prod"
    )
    d.update(ov)
...
def test_image_transfer_copy_failure_raises(ft_dirs):
    ...
    with patch.object(
        file_transfer, "_copy_file_with_retry", side_effect=OSError("disk")
    ):
        with pytest.raises(ValueError):
...
def test_annotation_transfer_copy_failure_raises(ft_dirs):
    ...
    with patch.object(
        ...
        with pytest.raises(ValueError):
            file_transfer.annotation_transfer(
                {"filename": "a"}, {}, ".xml", str(s), "a.xml"
            )
